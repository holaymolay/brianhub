...
export function buildBrianhubApiHelpMarkdown({ origin = 'https://brianhub.com', workspaceId = '<workspace-id>' } = {}) {
  ...
  return [
    ...
    'Admin UI note: these identities are presented as `Service workers`, but the API route family is still `/admin/service-accounts`.',
    ...
    '- Create or rotate a token with a label, optional expiration, and optional permission narrowing.',
    '- The raw token secret is shown once at create or rotate time only.',
    '- Existing tokens are metadata-only after issuance.',
    '- Service-account list rows include creator attribution and summary counts when available.',
    '',
    ...
    '- Human workspace results are membership-scoped.',
    '- Personal workspaces are owner-isolated.',
    '- Organization operating surfaces are not normal workspace picker entries.',
    '',
    ...
    '',
    'Organizations are separate operating entities, not workspaces. The web app opens them as their own surface.',
    '',
    'Minimal create organization request:',
    formatJsonBlock({
      name: 'Pipe Cam'
    }),
    '',
    'Minimal add organization member request:',
    formatJsonBlock({
