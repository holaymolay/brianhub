...
test('desktop sidebar uses per-section scrolling and exposes organizations', () => {
  ...
  assert.match(styles, /\.sidebar-section-body \{[\s\S]*min-height: 0;[\s\S]*overflow: hidden;[\s\S]*max-height: 100vh;[\s\S]*transition: max-height 180ms ease, opacity 140ms ease;/s);
  assert.match(styles, /\.sidebar-section\.is-collapsed \.sidebar-section-body \{[\s\S]*max-height: 0;[\s\S]*opacity: 0;[\s\S]*pointer-events: none;/s);
