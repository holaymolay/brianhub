...
test('admin console exposes service-worker, token, and workspace-grant controls', () => {
  ...
  assert.match(html, /id="admin-service-token-table-head"/);
  ...
  assert.match(html, /id="admin-service-grants-table-head"/);
...
test('admin console wiring includes service-account API helpers and event handlers', () => {
  ...
  assert.match(appScript, /function getAdminServiceTokenUsageSummary\(token\)/);
  assert.match(appScript, /function getAdminServiceTokenTimingSummary\(token\)/);
  assert.match(appScript, /const readiness = getServiceWorkerReadinessState\(selectedAccount\);/);
  assert.match(appScript, /const attention = getServiceWorkerAttentionState\(selectedAccount\);/);
...
test('admin console auto-selects existing service accounts and surfaces summary counts', () => {
  ...
  assert.match(appScript, /adminServiceTokenTableHead\?\.classList\.remove\('hidden'\);/);
  assert.match(appScript, /usagePrimary\.textContent = getAdminServiceTokenUsageSummary\(token\);/);
  assert.match(appScript, /usageSecondary\.textContent = getAdminServiceTokenTimingSummary\(token\);/);
  assert.match(appScript, /editBtn\.textContent = token\.id === adminState\.selectedServiceAccountTokenId \? 'Selected' : 'Edit';/);
  assert.match(appScript, /label: 'Operational status'/);
  assert.match(appScript, /value: operationalLabel,/);
  assert.match(appScript, /adminServiceSummaryBadges\.appendChild\(createAdminServiceChip\(readiness\.label, readiness\.tone\)\);/);
  assert.match(appScript, /adminServiceGrantsTableHead\?\.classList\.remove\('hidden'\);/);
  assert.match(appScript, /statusPrimary\.textContent = effective \? 'Effective now' : 'Inactive';/);
